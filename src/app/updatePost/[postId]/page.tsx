...
import { getPost, updatePost } from '@/lib/api/actions';
...
const UpdatePostPage = () => {
    ...
    if (!context) throw new Error("UpdatePostPage must be within AuthContextProvider");
    ...
    useEffect(() => {
        if (postData) {
            ...
            setPrice(postData.price || 0);
            setAddress(postData.address || "");
            setDesc(postData?.postDetail?.[0]?.desc || ""); 
            setCity(postData.city || "");
            setBathroom(postData.bathroom || 0);
            setBedroom(postData.bedroom || 0);
            setLatitude(postData.latitude || 0);
            setLongitude(postData.longitude || 0);
            setType(postData.type || "rent");
            setProperty(postData.property || "apartment");
            setUtilities(postData?.postDetail?.[0]?.utilities || "Owner is responsible"); 
            setPet(postData?.postDetail?.[0]?.pet || "Allowed");
            setIncome(postData?.postDetail?.[0]?.income || "");
            setSize(postData?.postDetail?.[0]?.size ?? 0); 
            setSchool(postData?.postDetail?.[0]?.school ?? 0);
            setBus(postData?.postDetail?.[0]?.bus ?? 0);
            setRestaurant(postData?.postDetail?.[0]?.restaurant ?? 0);
        }
    }, [postData]);
    ...
    return userData && postData ? (
        <div className='relative h-full flex max-md:flex-col-reverse max-md:bg-[#fcf5f3] rounded-md max-md:px-3'>
            ...
            <div className='flex-[2] md:bg-[#fcf5f3] flex flex-col gap-5 items-center md:mt-5'>
                <h1 className='text-lg md:text-2xl mt-3 md:mt-5'>Update images</h1>
                <label htmlFor="image1">
                    ...
                    <Image src={image1 ? URL.createObjectURL(image1) : postData.images?.[0]?.url || "/upload_images_area.svg"} alt="" width={500} height={500} className={`md:w-auto aspect-[2/1] md:h-45 border border-gray-300 rounded ${!image1 && !postData.images?.[0]?.url ? "opacity-40" : ""}`} />
                </label>
                ...
                <label htmlFor="image2">
                    ...
                    <Image src={image2 ? URL.createObjectURL(image2) : postData.images?.[1]?.url || "/upload_images_area.svg"} alt="" width={500} height={500} className={`md:w-auto aspect-[2/1] md:h-45 border border-gray-300 rounded ${!image2 && !postData.images?.[1]?.url ? "opacity-40" : ""}`} />
                </label>
                ...
                <label htmlFor="image3">
                    ...
                    <Image src={image3 ? URL.createObjectURL(image3) : postData.images?.[2]?.url || "/upload_images_area.svg"} alt="" width={500} height={500} className={`md:w-auto aspect-[2/1] md:h-45 border border-gray-300 rounded ${!image3 && !postData.images?.[2]?.url ? "opacity-40" : ""}`} />
                </label>
                ...
                <label htmlFor="image4">
                    ...
                    <Image src={image4 ? URL.createObjectURL(image4) : postData.images?.[3]?.url || "/upload_images_area.svg"} alt="" width={500} height={500} className={`md:w-auto aspect-[2/1] md:h-45 border border-gray-300 rounded ${!image4 && !postData.images?.[3]?.url ? "opacity-40" : ""}`} />
                </label>
            </div>
        </div>
    ) : (
